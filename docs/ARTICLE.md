# greenboost: Solving VRAM Limitations for Machine Learning Workloads
...
**greenboost** (Virtual GPU Memory Emulator) is a Linux kernel driver that addresses this limitation by extending available VRAM with system RAM. The project is particularly valuable for:
...
**greenboost offers a different approach**: extending video memory with system RAM through transparent CUDA integration.

## How greenboost Works: Architecture and Mechanisms
...
### 3. greenboost Operating Mechanism

greenboost operates at the Linux kernel level, intercepting VRAM requests:

```c
// Virtual GPU device structure
struct greenboost_device {
    // Configuration
    struct greenboost_config config;
    ...
    // Interfaces
    struct device *device;         // /dev/greenboost
...
int greenboost_vram_access(u64 offset, void *buffer, size_t size)
{
...
└────────────────────────────────────────────────┘
          ...
          ▼                           ▼
┌──────────────────┐        ┌──────────────────┐
│  Physical VRAM   │        │  greenboost Driver     │
│  GDDR6X          │        │  (System RAM)    │
│  12 GB           │        │  +2 GB           │
└──────────────────┘        └──────────────────┘
```
...
**With greenboost (+2 GB):**
```bash
# Load greenboost with 2 GB extension
sudo insmod greenboost.ko gpu_model=4070 physical_vram_gb=12 virtual_vram_gb=2
...
**Solution with greenboost:**

```bash
# Configure large extension
sudo insmod greenboost.ko gpu_model=4070 physical_vram_gb=12 virtual_vram_gb=48
# Total VRAM: 60 GB
```
...
training_args = TrainingArguments(
    per_device_train_batch_size=4,  # Without greenboost: max 1
...
# With greenboost (12 + 24 GB = 36 GB): ✅ Fits!
...
# Without greenboost: impossible to run
```
...
One of the most powerful applications of greenboost is creating multiple virtual GPUs for VMs:
...
### Setting up GPU Passthrough with greenboost

#### Step 1: Host Configuration

```bash
# /etc/greenboost/vgpu-profiles.conf
[vGPU-Profile-1]
gpu_model=4090
physical_vram_gb=6
virtual_vram_gb=2
...
# Verify
ls /sys/class/greenboost/
# greenboost0  greenboost1  greenboost2  greenboost3

# Check memory on each
for i in {0..3}; do
    echo "vGPU $i:"
    cat /sys/class/greenboost/greenboost$i/vram_info
done
```
...
<domain type='kvm'>
  ...
  <devices>
    ...
    <!-- greenboost device node -->
    <filesystem type='mount' accessmode='passthrough'>
      <source dir='/dev/greenboost0'/>
      <target dir='vgpu'/>
    </filesystem>
  </devices>
</domain>
...
│                                          │
│  greenboost: 32 virtual GPUs × 8 GB          │
│       = 32 isolated users               │
│                                          │
│  Usage:                                 │
...
services:
  dev-env-small:
    ...
    devices:
      - /dev/greenboost0:/dev/nvidia0  # 4 GB
    environment:
  ...
  dev-env-medium:
    ...
    devices:
      - /dev/greenboost1:/dev/nvidia0  # 8 GB
    environment:
      - CUDA_VISIBLE_DEVICES=0
  ...
  dev-env-large:
    ...
    devices:
      - /dev/greenboost2:/dev/nvidia0  # 16 GB
    environment:
      - CUDA_VISIBLE_DEVICES=0
```
...
RTX 4070 + greenboost (+2 GB)  | 0.156s    | 1641 img/s | 13.5 GB
RTX 4070 + greenboost (+4 GB)  | 0.171s    | 1497 img/s | 15.2 GB
...
+2 GB greenboost (batch=2)     | 3.8s     | 2     | 13.1 GB
+4 GB greenboost (batch=4)     | 4.9s     | 4     | 14.8 GB
...
#### 3. Scenarios Where greenboost Won't Help
...
✅ **Training**: Gradient checkpointing + greenboost = larger batches  
...
### 3. Gradient Accumulation + greenboost
...
greenboost maintains coherency between physical and virtual VRAM:

```c
// Write to virtual VRAM
void greenboost_write(u64 addr, void *data, size_t size)
{
...
void greenboost_read(u64 addr, void *buffer, size_t size)
{
    if (addr >= physical_vram_size) {
...
- Solution: greenboost with 4× virtual GPUs
...
**Economics**:
- Without greenboost: 4× RTX 4090 = $6400
- With greenboost: 1× RTX 4090 + RAM = $1800
- **Savings: $4600 (72%)**
...
# Load model with greenboost extended memory
generator = pipeline(
    "text-generation",
...
# Without greenboost: Model doesn't fit → INT8 quantization required
```
...
```c
// greenboost kernel module structure

module_init(greenboost_init);
module_exit(greenboost_exit);

// Initialization sequence:
static int __init greenboost_init(void)
{
    1. Parse module parameters (gpu_model, vram sizes)
    2. Validate GPU model against supported list
    3. Allocate greenboost_device structure
    4. Initialize configuration
    5. Create character device (/dev/greenboost)
    6. Register sysfs interface (/sys/class/greenboost/)
...
greenboost uses `vmalloc()` for large virtual VRAM allocations:
...
// greenboost hook
unsigned long greenboost_hooked_get_total_vram(void) {
    unsigned long physical = nvidia_get_total_vram();
    unsigned long virtual_vram = greenboost_config.virtual_vram_size;
...
**greenboost** provides a practical solution to one of the key challenges in modern machine learning - limited video memory. The project is especially valuable for:
...
```bash
# Clone repository
git clone https://github.com/yourusername/greenboost.git
cd greenboost
...
sudo insmod greenboost.ko gpu_model=4070 physical_vram_gb=12 virtual_vram_gb=4
...
**Links**:
- GitHub: [https://github.com/bogdanovby/greenboost](https://github.com/bogdanovby/greenboost)
- Documentation: /docs/
...
© 2024 greenboost Project Contributors | GPL-2.0 License
